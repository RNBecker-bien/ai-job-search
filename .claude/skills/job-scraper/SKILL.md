...
10. **Industry only, strictly entry-level.** Per the Employer Type Filter and Experience & Degree Filter in `search-queries.md`: never present a university/academic posting, and never present a posting requiring more than 3 years of experience or a Master's/PhD as a stated minimum. Still log excluded postings to `seen_jobs.json` with status `skipped` for dedup, same as any other filtered result.
11. **Verify the posting is genuinely from the employer it claims.** Before presenting a medium/high-fit match, check for signals that the aggregator listing doesn't actually match the named employer's own careers site - a different company name mentioned inside the posting body, a staffing-agency intermediary, or a subsidiary/franchise brand switch. When in doubt, note the discrepancy in the result rather than presenting it as a clean match.
